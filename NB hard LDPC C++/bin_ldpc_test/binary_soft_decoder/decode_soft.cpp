...
bool BitFlipping(LDPC& ldpc, vector<int>& x, int max_iter, int theta, vector<int>& y, int* number_of_iter)
{
    ...
    for (int loop = 0; loop < max_iter; ++loop)
    {
        ...
        for (int i = 0; i < ldpc.n; i++)
        {
            ...
            if ((2*unsatisfied - ldpc.col_weight[i]) > theta)
            {
            	y[i] ^= 1; // flip the bit
                for (int k = 0; k < ldpc.col_weight[i]; k++)
                {
                    R_msgs(ldpc.msgs_col(i,k)) ^= 1;
                }
            }
            for (int k = 0; k < ldpc.col_weight[i]; k++)
		    {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    switch(command)
    {
        ...
        case 9:
        {
            ...
            {
            result = BitFlipping(ldpc, x, iMaxNumberOfIterations, thetas[itr], y, &number_of_iter);
            if (result == 1)
            {
                break;
            }
