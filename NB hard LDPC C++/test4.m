%Running the  simulation for Q = 4, creating files for plotting
fe = 20;
snr = 1:0.5:8.5;
addpath encoder;
addpath decoder;
...
for qq = [4]
    if (qq == 4)
        for ii = 1:3
            for i = 5:7
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[0]);
            end
            if (ii == 1)
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[1,0]);
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[2,1,0]);
            elseif (ii == 2)
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[1,0]);
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[2,1,0]);
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[3,2,1,0]);
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[4,3,2,1,0]);
            elseif (ii == 3)
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[1,0]);
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[2,1,0]);
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[3,2,1,0]);
                ...
                simulate(matrix_q_4(ii,:),i, 10, snr, fe,[6,4,3,2,1,0]);
            end  
